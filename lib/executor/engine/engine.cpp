...
Expect<void> Executor::execute(Runtime::StackManager &StackMgr,
  ...
  auto Dispatch = [this, &PC, &StackMgr]() -> Expect<void> {
    ...
    auto toTopHeapType = [&StackMgr](const ValType &T) -> ValType {
      ...
      if (T.isAbsHeapType()) {
        ...
        const auto &CompType =
            (*StackMgr.getModule()->getType(T.getTypeIndex()))
                ->getCompositeType();
        if (CompType.isFunc()) {
          return TypeCode::NullFuncRef;
        } else {
    ...
    case OpCode::Ref__null:
      return runRefNullOp(StackMgr, toTopHeapType(Instr.getValType()));
