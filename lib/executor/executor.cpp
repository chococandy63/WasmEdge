  ...
  if (ModInst) {
    ...
    for (uint32_t I = 0; I < RTypes.size(); ++I) {
      ...
      if (RTypes[RTypes.size() - I - 1].isRefType()) {
        Returns[RTypes.size() - I - 1] = std::make_pair(
            Val, ModInst->expandValType(Val.get<RefVariant>().getType()));
      } else {
        Returns[RTypes.size() - I - 1] = std::make_pair(
            Val, ModInst->expandValType(RTypes[RTypes.size() - I - 1]));
      }
    }
  } else {
    // Independent function instance case. This function will always be a host
    // function, and only have abstract heap types for returns.
    for (uint32_t I = 0; I < RTypes.size(); ++I) {
      auto Val = StackMgr.pop();
