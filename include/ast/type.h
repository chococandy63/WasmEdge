...
/// TODO: GC - refine the matcher and RTT type matcher.
class TypeMatcher {
public:
  /// Validation: Match 2 defined types in the same module.
  static bool matchType(Span<const SubType *const> TypeList, uint32_t ExpIdx,
                        uint32_t GotIdx) noexcept {
    if (ExpIdx >= TypeList.size() || GotIdx >= TypeList.size()) {
      return false;
    }
    if (isDefTypeEqual(TypeList, ExpIdx, GotIdx)) {
      return true;
    }
    const auto *GotType = TypeList[GotIdx];
    for (auto TIdx : GotType->getSuperTypeIndices()) {
      if (matchType(TypeList, ExpIdx, TIdx)) {
        return true;
      }
    }
    return false;
  ...
  /// Validation: Match 2 composite types in the same module.
  static bool matchType(Span<const SubType *const> TypeList,
    ...
    case TypeCode::Func: {
      ...
      return matchTypes(TypeList, ExpFType.getParamTypes(),
  ...
  /// Validation: Match 2 value types in the same module.
  static bool matchType(Span<const SubType *const> TypeList, const ValType &Exp,
    ...
    if (Exp.isRefType() && Got.isRefType()) {
      ...
      if (Exp.isAbsHeapType() && Got.isAbsHeapType()) {
        ...
        if (Got.getTypeIndex() >= TypeList.size()) {
          return false;
        }
        return matchTypeCode(
            Exp.getHeapTypeCode(),
            TypeList[Got.getTypeIndex()]->getCompositeType().expand());
      } else if (Got.isAbsHeapType()) {
        // Case 3: Match abstract heap type to a type index.
        if (Exp.getTypeIndex() >= TypeList.size()) {
          return false;
        }
        TypeCode ExpandGotType =
            TypeList[Exp.getTypeIndex()]->getCompositeType().expand();
        ...
        return matchType(TypeList, Exp.getTypeIndex(), Got.getTypeIndex());
      }
    }
    return false;
  }

  /// RTT matcher: Match 2 defined types in different modules.
  ...
  /// RTT matcher: Match 2 value types in different modules.
  ...
  /// Validation: Match 2 type lists in the same module.
...
private:
  /// Validation: Helper for checking the equivalent of 2 defined types.
  static bool isDefTypeEqual(Span<const SubType *const> TypeList,
                             uint32_t LHSIdx, uint32_t RHSIdx) {
    if (LHSIdx == RHSIdx) {
      // Two type indices are the same.
      return true;
    }
    const auto *LHSType = TypeList[LHSIdx];
    const auto *RHSType = TypeList[RHSIdx];
    ...
    if (LRecSize > 1) {
      ...
      return isRecTypeEqual(TypeList, LStartIdx, RStartIdx, LRecSize);
    } else {
      // Both are composite types or self-recursive types.
      return isRecTypeEqual(TypeList, LHSIdx, RHSIdx, 1);
    }
  }

  /// Validation: Helper for checking the equivalent of 2 recursive types.
  static bool isRecTypeEqual(Span<const SubType *const> TypeList,
                             uint32_t LStartIdx, uint32_t RStartIdx,
                             uint32_t RecSize) {
    ...
    auto isValTypeEqual = [&](const ValType &LType,
      ...
      if (LType.getHeapTypeCode() == TypeCode::TypeIndex &&
        ...
        assuming(LIdx < TypeList.size() && RIdx < TypeList.size());
        ...
        // defined types.
        return isDefTypeEqual(TypeList, LIdx, RIdx);
      } else {
    ...
    for (uint32_t I = 0; I < RecSize; I++) {
      ...
      const auto *LHSType = TypeList[LStartIdx + I];
      const auto *RHSType = TypeList[RStartIdx + I];
      ...
      /*
      // TODO: GC - compare the supertype indices.
      uint32_t SuperTypesSize = LSuperTypes.size();
      for (uint32_t I = 0; I < SuperTypesSize; I++) {
        if (!isDefTypeEqual(TypeList, LSuperTypes[I], RSuperTypes[I])) {
          return false;
        }
      }
      */
  ...
  /// RTT matcher: Helper for checking the equivalent of 2 defined types.
  ...
  /// RTT matcher: Helper for checking the equivalent of 2 recursive types.
  static bool isRecTypeEqual(Span<const SubType *const> LHSList,
    ...
    for (uint32_t I = 0; I < RecSize; I++) {
      ...
      uint32_t SuperTypesSize = LSuperTypes.size();
      for (uint32_t I = 0; I < SuperTypesSize; I++) {
        if (!isDefTypeEqual(LHSList, LSuperTypes[I], RHSList, RSuperTypes[I])) {
          return false;
        }
      }
      */
  ...
  /// Helper for matching 2 type codes.
